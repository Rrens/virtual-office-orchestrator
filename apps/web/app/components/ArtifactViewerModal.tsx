    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-4xl bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800 flex-shrink-0">
          <div>
            <h2 className="text-white font-bold text-base">Artifacts</h2>
            {taskTitle && <p className="text-slate-400 text-xs mt-0.5 truncate">{taskTitle}</p>}
          </div>
          <button
            onClick={onClose}
            ...
            <div className="w-56 flex-shrink-0 border-r border-slate-800 overflow-y-auto p-3 space-y-1">
              {artifacts.map((a) => (
                ...
                >
                  ...
                  <p className="text-slate-500 mt-0.5">{formatBytes(a.sizeBytes)}</p>
                  {a.agentInstance && (
                    <p className="text-slate-500 truncate mt-0.5">{a.agentInstance.definition.name}</p>
                  )}
                </button>
              ))}
            </div>
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between px-4 py-2 border-b border-slate-800 bg-slate-900/50 flex-shrink-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs px-1.5 py-0.5 rounded bg-slate-800 text-slate-400">{selected.mimeType}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={handleCopy}
                ...
                <div className="flex-1 overflow-auto p-4">
                  {selected.content ? (
                    ...
                    <p className="text-slate-500 text-sm">Content not available.</p>
                  )}
                </div>
              </div>
            )}
